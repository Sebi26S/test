...
namespace RTS.Commands
{
    ...
    public class BuildingRestrictionSO : ScriptableObject
    {
        ...
        public bool CanPlace(Vector3 position, Owner owner)
        {
            int hits = HitDetectionStyle switch
            {
                ...
                OverlapStyle.Vision => IsVisibleForOwner(position, owner) ? 0 : 1,
                _ => 1
            };
        ...
        private bool IsVisibleForOwner(Vector3 position, Owner owner)
        {
            FogVisibilityManager manager = FogVisibilityManager.Get(owner);

            if (manager == null)
            {
                Debug.LogWarning($"No FogVisibilityManager found for owner {owner}.");
                return false;
            }

            return manager.IsVisible(position);
        }
